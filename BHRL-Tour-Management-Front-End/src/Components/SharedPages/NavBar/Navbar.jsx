...
const Navbar = () => {
  ...
  const linkStyle =
    'hover:text-green-200 transition duration-300 relative';
  const activeStyle = 'text-yellow-400 after:absolute after:-bottom-1 after:left-0 after:w-full after:h-0.5 after:bg-yellow-400';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="hidden md:flex items-center space-x-6 font-semibold">
            {navLinks.map(({ name, path }) => (
              <NavLink
                key={name}
                to={path}
                className={({ isActive }) => isActive ? `${linkStyle} ${activeStyle}` : linkStyle}
              >
                {name}
              </NavLink>
            ))}
            ...
            <div className="relative">
              <button onClick={toggleUserDropdown} className="text-white text-2xl focus:outline-none hover:text-green-200">
                <FaUserCircle />
              </button>
              {userDropdown && (
          ...
          <div className="md:hidden flex items-center">
            <button onClick={toggleMenu} className="text-white hover:text-green-200 focus:outline-none">
              {isOpen ? (
        ...
        <div className="md:hidden bg-green-800 px-4 py-2 space-y-1 text-white font-semibold">
          {navLinks.map(({ name, path }) => (
            <NavLink
              ...
              to={path}
              onClick={toggleMenu}
              className={({ isActive }) => isActive ? `${linkStyle} ${activeStyle} block py-2` : `${linkStyle} block py-2`}
            >
              {name}
            </NavLink>
          ))}
